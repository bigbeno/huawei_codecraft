...
//                                        sizeof(hashmap_elem_t));
    ...
    m->elems = (hashmap_elem_t *)malloc(HMAP_INITIAL_SIZE * sizeof(hashmap_elem_t));
    if (!m->elems) {
        free(m);
        exit(HMAP_E_OUTMEM);
    }
    memset(m->elems, 0, HMAP_INITIAL_SIZE * sizeof(hashmap_elem_t));
...
int hashmap_put(hmap_t in, char* key, void_ptr value) {
    ...
    if (elem->key != NULL)
        free(elem->key);
    elem->key = strdup(key); /* only set to a reference */
